...
export const CollNodeSvcDb = {
    removeBmsChPositions: async (memberId, bmIds: string[], tx: Transaction): Promise<void> => {
        // move bms to bmLooseContainer
        // set parentMeta.childPositions to [folderIds]
        await tx.run(`
            MATCH (:Member {id: $memberId})-->(b:Bookmark)
            WHERE b.id IN $bmIds
            MATCH (b)<-[:CONTAINS]-(p)
            ...
            return 1 as r
        `, {bmIds: bmIds, memberId}); // Note that we're now passing an array of ids
    },
    moveBmsToBLC: async (memberId, bmIds: string[], tx: Transaction): Promise<number> => {
        // move bms to bmLooseContainer
        return (await tx.run(`
            MATCH (:Member {id: $memberId})-->(b:Bookmark)
            WHERE b.id IN $bmIds
            ...
            RETURN 1 as r
        `, {bmIds: bmIds, memberId})).records[0].get('r').toNumber(); // Note that we're now passing an array of ids
    },
    moveParentBmsDeepToBLC: async (memberId, parentIds: string[], tx: Transaction): Promise<number> => {
        // move bms to bmLooseContainer
        return (await tx.run(`
            MATCH (:Member {id: $memberId})-->(topP:Parent)
            ...
            RETURN 1 as r
        `, {parentIds: parentIds, memberId})).records[0].get('r').toNumber();
    },
    removeBmsChPositionsDeep: async (memberId, parentIds: string[], tx: Transaction): Promise<number> => {
        // move bms to bmLooseContainer
        // set parentMeta.childPositions to [folderIds]
        return (await tx.run(`
            MATCH (:Member {id: $memberId})-->(topP:Parent)
